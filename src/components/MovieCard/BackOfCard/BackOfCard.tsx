...
import { correctRating, correctReleaseDate } from "../../../utils/utils"

const BackOfCard: FC<BackOfCardProps> = ({ movieRating, movieTitle, releaseDate }) => {
    ...
    return (
        <div className='absolute my-rotate-y-180 backface-hidden rounded-3xl h-full w-full'>
            <div className='w-full h-full rounded-3xl flex flex-col justify-between items-center shadow-[1px_1px_42px_4px_rgba(140,140,140,0.41)]'>
                <h1 className='font-extrabold text-orange-600 pt-4'>{movieTitle}</h1>
                <div className='flex w-full justify-between px-6'>
                    <div className='flex flex-col justify-between items-center h-[80px]'>
                        <span className='font-light text-sm'>Rating: </span>
                        <span className='block h-[43px] w-[43px] border-2 border-green-600 p-2 rounded-full'>{correctRating(movieRating)}</span>
                    </div>
                    <div className='flex flex-col justify-between items-center h-[80px]'>
                ...
                <div className='flex flex-col justify-center items-center'>
                    ...
                    <span className='text-xs font-light'>{correctReleaseDate(releaseDate)}</span>
                </div>
